...
function fileValid(){
	...
	//let isalid= true;
	//button 타입은 submit 으로 하고 form 태그의 onsubmit 속성에 유효성 검사 함수를 설정할 때는 isValid 변수는 필요없습니다..
	...
	if(imgfile.value == "" || imgfile.value == null){
		...
		if(!imgfile.value.match(/(.*?).(jpg|png|gif|jpeg)$/)){	//정규표현식으로 확장자 검사
			alert("파일 형식은 jpg,png,gif만 가능합니다.")
			//isValid = false;
			return false;
		}
		if(imgfile.files[0].size > maxSize){	
			//imgfile.value 는 선택한 파일명, imgfile.files[0] 다중선택가능하므로 파일 자체를 인덱스로 지정합니다.
...
<input type="file" name="pic" accept="image/jpeg" id="imgfile">
	<!-- 웹에서는 파일의 형식을 확장자로 구분하지 않고 MIME 형식을 사용합니다.
		image/* , image/*png,image/jpeg , video/* 등등
		accept 속성은 파일 선택 창에서 초기 형식. -> *.* 모든 파일로 변경 및 파일 전송도 가능.
	 -->
	<button>전송</button>
</form>
<!-- 
	...
	2. 서버 페이지에서는 file 타입 요소로 전달된 파일을 다른 파라미터(입력값)과 함께 전송받습니다. (업로드)
		ㄴ > 서버에서 업로드 폴더 설정이 필요(예시 : D:\KSB\upload)
